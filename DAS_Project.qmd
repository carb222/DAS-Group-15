...
```{r}
#Remove entries in 'clean_data' where 'altitude_mean_meters' is greater than 9000
clean_data<-clean_data %>%
...
g1 <- ggplot(data = clean_data, aes(x = Qualityclass, y = aroma, fill = Qualityclass)) +
  geom_boxplot() +
...
g2 <- ggplot(data = clean_data, aes(x = Qualityclass, y = flavor, fill = Qualityclass)) +
  geom_boxplot() +
  labs(x = "Qualityclass", y = "flavor") + 
  theme(legend.position = "none") +
  coord_fixed(ratio = 1) # Ensuring the plot is square
# Create a boxplot of 'acidity' by 'Qualityclass' without a legend
g3 <- ggplot(data = clean_data, aes(x = Qualityclass, y = acidity, fill = Qualityclass)) +
  geom_boxplot() +
...
ggplot(data = clean_data, aes(x = Qualityclass, y = altitude_mean_meters , fill = Qualityclass)) +
  geom_boxplot() +
...
ggplot(data = clean_data, aes(x = Qualityclass, y = category_two_defects , fill = Qualityclass)) +
  geom_boxplot() +
...
ggplot(clean_data,aes(x=harvested,group=Qualityclass,fill=Qualityclass))+geom_bar()
```
...
ggplot(clean_data, aes(x = country_of_origin, group = Qualityclass, fill = Qualityclass)) +
  geom_bar() +
...
ggpairs(clean_data1, columns = columns_to_show, mapping = ggplot2::aes(colour = Qualityclass))
```
...
clean_data$Qualityclass <- factor(clean_data$Qualityclass,levels = c("Poor","Good"))
